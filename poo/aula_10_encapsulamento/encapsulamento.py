class ContaBancaria:
    """
    Cria uma conta bacária e permite fazer saques e depósitos_
    """
    def __init__(self, id, nome, saldo=0):
        self.id = id # publico (+)
        self._titular = nome # protegido (#)
        self.__saldo = saldo # privado (-)
        print(f'Conta {self.id} criada com sucesso. Saldo atual de R${self.__saldo:,.2f}')

    def __str__(self):
        return f'A conta {self.id} de {self._titular} tem R${self.__saldo:,.2f} de saldo.'
    
    def depositar(self, valor):
        self.__saldo += abs(valor) # abs() permite transformar valores negativos em positid
        print(f'Depósito de R${valor:,.2f} autorizado na conta {self.id}.')

    def sacar(self, valor):
        if valor > self.__saldo:
            print(f'Saque NEGADO de R${valor:,.2f} na conta {self.id}: SALDO INSUFICIENTE')
        else:
            self.__saldo -= abs(valor)
            print(f'Saque de R${valor:,.2f} autorizado na conta {self.id}.')


c1 = ContaBancaria(122, 'Gustavo', 30000)
c1.depositar(500)
print(c1)
c1.sacar(2000000)
print(c1)